...
> 当前实现：`admin/` 使用 React 19、Ant Design 6 与响应式 CSR 路由。文中标为“建议接口”的能力仍不属于当前 API。

## 1. 结论

后台管理端面向唯一超级管理员，用来维护调用方与 API Key、控制平台状态和凭据、查看解析日志与统计、执行受控平台测试，并完成管理员自身的安全操作。

界面采用黑白中性色和克制的功能色，保持较高信息密度。桌面端使用固定工作区和可折叠侧栏，移动端将导航改为抽屉；列表、筛选和详情始终优先保证完整业务信息，不通过截断原始 JSON 来掩盖布局问题。
...
![管理后台运行概览](admin-dashboard-desktop.png)

调用日志（桌面端，含调用 IP 与分页）：

![管理后台调用日志](admin-logs-desktop.png)

移动端平台管理与调用日志：

<p align="center">
  <img src="platform-management-mobile.png" width="360" alt="移动端平台管理">
  <img src="admin-logs-mobile.png" width="360" alt="移动端调用日志">
</p>

> 截图使用隔离演示数据，不包含真实 API Key、Cookie 或管理员凭据。
...
7. **组件使用公开 API**：当前 Ant Design 6 应用使用一个根 `ConfigProvider` 和主题 Token，不依赖 `.ant-*` 内部选择器。
...
筛选条件写入 URL 查询参数，但不得把输入全文或响应写入 URL。当前日志 API 使用服务端页码分页，支持每页 20、50 或 100 条并返回真实总数；兼容调用方仍可继续使用旧的 `cursor`/`limit` 参数。
...
当前实现为 Ant Design 6 CSR 应用，使用以下公开组件 API：
...
| Token | 值 | 用途 |
| --- | --- | --- |
| `colorPrimary` | `#18181B` | 主按钮、品牌标记和选中状态 |
| `colorCanvas` | `#FAFAFA` | 页面与侧栏背景 |
| `colorSurface` | `#FFFFFF` | 卡片与表格背景 |
| `colorSurfaceMuted` | `#F5F5F5` | 筛选、图标和悬停表面 |
| `colorSuccess` | `#16A34A` | 成功、已就绪 |
| `colorWarning` | `#D97706` | 待处理、即将过期 |
| `colorError` | `#DC2626` | 失败、吊销、删除 |
| `colorInfo` | `#2563EB` | 中性信息与链接 |
| `colorBorder` | `#E5E5E5` | 边框与分隔线 |
| `colorText` | `#171717` | 主文字 |
| `colorTextSecondary` | `#525252` | 次级文字 |
...
| 阴影 | `0 1px 2px rgb(0 0 0 / 4%)` |

黑白中性色用于主体层级，成功、警告、错误和信息色只用于对应业务状态；危险操作始终使用错误色，不能只依赖颜色表达。
...
│               │ └─────────────────────────────────────────────────────────┘ │
│               │ ┌─────────────────────────────────────────────────────────┐ │
│               │ │ 时间 │ Request ID │ 调用方 │ 平台 │ 状态 │ IP │ 耗时 │ │
│               │ │ ...                                                   │ │
│               │ │ ...                                                   │ │
│               │ └─────────────────────────────────────────────────────────┘ │
│               │              第 1–20 条，共 N 条  [1] [2] [20 条/页]      │
└───────────────┴─────────────────────────────────────────────────────────────┘
```
...
| 日志列表 | `GET /api/admin/v1/logs` | 多条件筛选、`page`/`page_size`/`total` 分页；兼容旧游标参数 |
...
- [ ] 强制改密期间无法进入其他管理页面。
- [ ] 日志列表使用服务端页码分页，展示真实总数，筛选变化后回到第一页。
...
- [ ] 已实现界面与仍属建议的接口有清晰标记，不以演示数据代替生产能力。
- [ ] 变更时仍需基于当前 OpenAPI 和 Ant Design 版本核对接口与组件 API。
